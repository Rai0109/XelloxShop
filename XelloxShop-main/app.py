...
from semantic_search import load_model, build_index, load_index, search_similar, add_training_pair, get_stats

# Chatbot globals
chat_index_loaded = False

def ensure_chat_training_data(db):
    """Ensure chat_training_data exists and seed initial pairs"""
    if "chat_training_data" not in db:
        db["chat_training_data"] = [
            {"question": "số dư", "answer": "💰 Kiểm tra số dư tại /profile"},
            {"question": "nạp", "answer": "💳 **Hướng dẫn nạp:**\\n```TK: 0967427517\\nTên: HO NHAT TRUONG\\nNgân hàng: MB Bank```\\nGhi VD: NAPusername123456\\nKiểm tra: /topup"},
            {"question": "nạp tiền", "answer": "💳 **Hướng dẫn nạp:**\\n```TK: 0967427517\\nTên: HO NHAT TRUONG\\nNgân hàng: MB Bank```\\nGhi VD: NAPusername123456\\nKiểm tra: /topup"},
            {"question": "mua hàng", "answer": "🛒 **Mua hàng:**\\n1. Nhấn **Thêm giỏ** / **Mua ngay**\\n2. Vào **Giỏ hàng** (/cart)\\n3. **Thanh toán** bằng xu\\nXem đơn: /orders"},
            {"question": "hỗ trợ", "answer": "📞 **Hỗ trợ:**\\nEmail: truong77kk@gmail.com\\nZalo: 0967427517"},
            {"question": "giới thiệu", "answer": "👥 **Giới thiệu bạn bè:** Chia sẻ mã referral tại /referral, nhận thưởng 10k xu/người!"},
            {"question": "admin", "answer": "🔧 **Admin:** Liên hệ chủ shop để hỗ trợ kỹ thuật."},
            {"question": "hello", "answer": "Xin chào! 🤖 Tôi đang học hỏi từ các cuộc trò chuyện. Hỏi tôi bất cứ điều gì về shop!"},
            {"question": "hi", "answer": "Chào bạn! 💬 Gõ câu hỏi để tôi hỗ trợ."},
            {"question": "sản phẩm hot", "answer": "🔥 **Sản phẩm hot:** Bot Zalo, Bot Facebook, Discord Bump. Xem tại trang chủ!"},
            # Vietnamese variations
            {"question": "tôi muốn mua", "answer": "🛒 **Mua hàng:**\\n1. Nhấn **Thêm giỏ** / **Mua ngay**\\n2. Vào **Giỏ hàng** (/cart)\\n3. **Thanh toán** bằng xu\\nXem đơn: /orders"},
            {"question": "làm sao nạp tiền", "answer": "💳 **Hướng dẫn nạp:**\\n```TK: 0967427517\\nTên: HO NHAT TRUONG\\nNgân hàng: MB Bank```\\nGhi VD: NAPusername123456\\nKiểm tra: /topup"},
            {"question": "số xu", "answer": "💰 Kiểm tra số dư tại /profile"},
            {"question": "đơn hàng", "answer": "📦 Xem đơn hàng tại /orders"},
            {"question": "giỏ hàng", "answer": "🛒 Vào /cart để xem và thanh toán"},
        ]
    
    # Mine historical user chats for training
    historical_pairs = []
    for user in db.get("users", []):
        chats = user.get("chats", [])
        for i in range(0, len(chats)-1, 2):  # User msg -> bot reply pairs
            if i+1 < len(chats):
                q = chats[i]["content"].lower().strip()
                a = chats[i+1]["content"]
                if q and a and len(q) > 1:
                    historical_pairs.append({"question": q, "answer": a})
    
    # Dedup and add unique
    existing_qs = {p["question"] for p in db["chat_training_data"]}
    for pair in historical_pairs:
        if pair["question"] not in existing_qs:
            db["chat_training_data"].append(pair)
            existing_qs.add(pair["question"])
    
    if len(db["chat_training_data"]) > 100:  # Limit initial
        db["chat_training_data"] = db["chat_training_data"][:100]

def init_chatbot():
    global chat_index_loaded
    if chat_index_loaded:
        return
    
    db = load_db()
    ensure_chat_training_data(db)
    save_db(db)
    
    # Build/load index
    if not load_index():
        build_index(db["chat_training_data"])
    
    chat_index_loaded = True
    logger.info("🤖 Chatbot initialized")

# Chatbot keyword fallback
KEYWORD_RESPONSES = {
    "số dư|balance|xu": "💰 Kiểm tra số dư tại /profile",
    "nạp|topup|nap|chuyen khoan": "💳 **Hướng dẫn nạp:**\\n```TK: 0967427517\\nTên: HO NHAT TRUONG\\nNgân hàng: MB Bank```\\nGhi VD: NAPusername123456\\nKiểm tra: /topup",
    "mua|mua hàng|san pham|product": "🛒 **Mua hàng:**\\n1. Nhấn **Thêm giỏ** / **Mua ngay**\\n2. Vào **Giỏ hàng** (/cart)\\n3. **Thanh toán** bằng xu\\nXem đơn: /orders",
    "hỗ trợ|support|help|cs|kh|kien thuc": "📞 **Hỗ trợ:**\\nEmail: truong77kk@gmail.com\\nZalo: 0967427517",
    "giới thiệu|referral|invite": "👥 **Giới thiệu bạn bè:** Chia sẻ mã referral tại /referral, nhận thưởng 10k xu/người!",
    "don hang|order|don|lich su": "📦 Xem đơn hàng tại /orders",
    "gio hang|cart": "🛒 Vào /cart để xem và thanh toán",
    "admin|quan tri": "🔧 **Admin:** Liên hệ chủ shop để hỗ trợ kỹ thuật.",
    "hello|hi|chao|xin chao": "Xin chào! 🤖 Tôi đang học hỏi từ các cuộc trò chuyện. Hỏi tôi bất cứ điều gì về shop!",
}
...
@rate_limit("chat", 10, 30)  # 10 msgs/30s
def api_chat():
    ...
    if not message or len(message) < 1:
        return jsonify({"ok": False, "msg": "Tin nhắn rỗng"})
    
    username = session.get("user", "guest")
    timestamp = datetime.datetime.now().strftime("%d/%m %H:%M")
    
    db = load_db()
    user = next((u for u in db["users"] if u["username"] == username), None) if username != "guest" else None
    
    # Search similar first
    similar = search_similar(message)
    if similar:
        response = similar[0]["answer"]  # Best match
        logger.info(f"🤖 Similarity match: {similar[0]['similarity']:.2f}")
    else:
        # Keyword fallback
        response = get_keyword_response(message)
        if not response:
            response = "🤖 Đang học hỏi... Cảm ơn bạn đã chat! Tôi sẽ nhớ để trả lời tốt hơn lần sau. Hoặc thử: số dư, nạp tiền, mua hàng."
    
    # Add to history if logged in
    if user:
        user_chat = user.setdefault("chats", [])
        user_chat.append({"is_bot": False, "content": message, "timestamp": timestamp})
    ...
    # Add bot response to history if logged in
    if user:
        user_chat.append({"is_bot": True, "content": response, "timestamp": timestamp})
        
        # Train on this pair
        add_training_pair(message, response)
        
        # Limit history
        if len(user_chat) > 200:
            user_chat[:] = user_chat[-200:]
